...
def round_coordinates(coordinates):#圆形坐标
    """Gives the integral rest."""
    coordinates = coordinates - np.floor(coordinates)
    return coordinates

def coordinate_wise_random_rounding(coordinates):
    """Rounds coordinates randonmly."""#随机化圆形坐标
...
class CKKSEncoder:
    ...
    def __init__(self, poly_degree: int, scale: float):
        ...
        """
        self.xi = np.exp(2 * np.pi * 1j / poly_degree)#单位的第m次根
        self.poly_degree = poly_degree #多项式的阶数
...
# 明文向量的长度为len(m)
# X^N + 1：N即为多项式阶：self.poly_degree = N = 4 * len(m)
# 编码后的明文长度为：N / 2
# a=np.array([1,2,3,4,5,6,7,8])
# encode=CKKSEncoder(32,pow(2,20))# 生成多项式系数个数是：n/2
# print(encode.encode(a))
# print(encode.decode(encode.encode(a)))
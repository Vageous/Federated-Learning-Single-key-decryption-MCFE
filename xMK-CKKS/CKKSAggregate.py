...
class CKKSAggregate:
    ...
    def aggregate(self,ciphertext,C_sum):
        ...
        if len(C_sum) == 0:
            ...
            C_sum[1] = Polynomial(utils.crange(((C_sum[1] + c1).coef % self.cipher_modulus), self.cipher_modulus))
        return C_sum
    ...
    def aggrergate1(self,ciphertext,C_sum):
        for i in range(len(ciphertext)):
            ...
            if len(C_sum[i]) ==0:
                ...
                C_sum[i][1]=Polynomial(utils.crange(((C_sum[i][1] + c1).coef % self.cipher_modulus), self.cipher_modulus))
